...
describe("BOM Handling", function () {
    ...
    it("strips UTF-16 BOM", function () {
        var body = Buffer.concat([
            utf16leBOM,
            iconv.encode(sampleStr, "utf16le"),
        ]);
        ...
        var body = Buffer.concat([
            utf16beBOM,
            iconv.encode(sampleStr, "utf16be"),
        ]);
        assert.equal(iconv.decode(body, "utf16"), sampleStr);
    ...
    it("doesn't strip BOMs when stripBOM=false", function () {
        var body = Buffer.concat([utf8BOM, new Buffer(sampleStr)]);
        assert.equal(
            iconv.decode(body, "utf8", { stripBOM: false }),
            strBOM + sampleStr
        );
        ...
        assert.equal(
            iconv.decode(body, "utf16", { stripBOM: false }),
            strBOM + sampleStr
        );
        assert.equal(
            iconv.decode(body, "utf16le", { stripBOM: false }),
            strBOM + sampleStr
        );
        ...
        assert.equal(
            iconv.decode(body, "utf16be", { stripBOM: false }),
            strBOM + sampleStr
        );
    });
    ...
    it("adds UTF-8 BOM when addBOM=true", function () {
        var body = Buffer.concat([utf8BOM, new Buffer(sampleStr)]).toString(
            "hex"
        );
        assert.equal(
            iconv.encode(sampleStr, "utf8", { addBOM: true }).toString("hex"),
            body
        );
    });
    ...
    it("adds UTF-16 BOMs when addBOM=true", function () {
        ...
        assert.equal(
            iconv
                .encode(sampleStr, "utf16le", { addBOM: true })
                .toString("hex"),
            body
        );
        ...
        assert.equal(
            iconv
                .encode(sampleStr, "utf16be", { addBOM: true })
                .toString("hex"),
            body
        );
    });
    ...
    it("'UTF-16' encoding adds BOM by default, but can be overridden with addBOM=false", function () {
        ...
        var body = Buffer.concat([iconv.encode(sampleStr, "utf16le")]).toString(
            "hex"
        );
        assert.equal(
    ...
    it("when stripping BOM, calls callback 'stripBOM' if provided", function () {
        ...
        var body = Buffer.concat([utf8BOM, new Buffer(sampleStr)]);
        assert.equal(
            iconv.decode(body, "utf8", { stripBOM: stripBOM }),
            sampleStr
        );
        assert(bomStripped);
