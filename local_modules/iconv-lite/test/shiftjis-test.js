...
describe("ShiftJIS tests", function () {
    it("ShiftJIS correctly encoded/decoded", function () {
        var testString = "中文abc", //unicode contains ShiftJIS-code and ascii
            testStringBig5Buffer = new Buffer([
                0x92, 0x86, 0x95, 0xb6, 0x61, 0x62, 0x63,
            ]),
            testString2 = "測試",
        ...
        assert.strictEqual(
            iconv.decode(testStringBig5Buffer, "shiftjis"),
            testString
        );
        assert.strictEqual(
    ...
    it("ShiftJIS extended chars are decoded, but not encoded", function () {
        ...
        assert.strictEqual(
            iconv.encode(str2, "shiftjis").toString("hex"),
            res2
        );
    });
    ...
    it("ShiftJIS includes extensions", function () {
        assert.strictEqual(
            iconv.decode(new Buffer("8740", "hex"), "shiftjis"),
            "①"
        );
        assert.strictEqual(
            iconv.encode("①", "shiftjis").toString("hex"),
            "8740"
        );
    });
});
