...
const collectionQueriesUpdateBadges:any = [
    {
        ...
        match: {"region": {"$in" : ["abitibi-temiscamingue", "north Ontario", "baies-james"]}},
        queries: [
            queryAddBadgeCBToBadgesArray
        ]
    },
    {
        collection: "people",
...
export async function up(): Promise<void> {
    ...
    await runQueriesOnDatabase(driver, 'bdsol-data', collectionQueriesUpdateBadges, 'Updating people and organisations that are "not in other region" (in CB) with CB badge', 'up');
}
