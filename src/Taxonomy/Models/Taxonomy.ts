...
class Taxonomy extends AbstractModel {
    ...
    public static getInstance(doIndexes = true): Taxonomy {
        if (Taxonomy._instance === undefined) {
            ...
            Taxonomy._instance.schema.virtual("type").get(function () {
                return Taxonomy._instance.modelName;
            });
    ...
    schema: Schema = new Schema<TaxonomySchema>(
        {
            category: {
                type: String,
                ...
                type: [
                    {
                        domain: {
                            ...
                            validate: function (
                                ...
                                if (
                                    value &&
                                    value.equals(currentDocument._id)
                                ) {
                                    throw new Error(
                                        "Can't assign self as a domain."
                                    );
                                }
                                return true;
                            },
