...
export abstract class BaseProvider implements DbProvider {
    ...
    public async connect(): Promise<mongoose.Connection | boolean> {
        ...
        if (!this.isConnected) {
            ...
            if (this.verbose)
                LogHelper.info(
                    `[BD] Connecting to mongo url ${this._driver.urlToLog(url)} on ${this._databaseName} database`
                );
            try {
                ...
                if (typeof this.connection !== "undefined") {
                    ...
                    if (this.verbose)
                        LogHelper.info(
                            `[BD] Connection succeed to mongo url ${this._driver.urlToLog(url)} on ${this._databaseName} database with this state ${this.connection.readyState}`
                        );
    ...
    public async createMongooseConnection(): Promise<Connection | undefined> {
        ...
        try {
            ...
            LogHelper.error(
                `[DB][createMongooseConnection] can't create connection to mongo server with mongoose  on ${this._databaseName}`,
                error
            );
            return;
        }
    ...
    public assign(service: Service): void {
        try {
            ...
            if (this.verbose)
                LogHelper.info(
                    `[DB][${service.constructor.name}] CONNECTION TO Mongoose ${succeedConnection ? "succeed" : "failed"}`
                );
