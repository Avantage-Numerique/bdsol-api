...
class OrganisationsController extends AbstractController {
    ...
    public async aggregateSingle(slug: any): Promise<ApiResponseContract> {
        ...
        let query: Array<any> = lookupModelsByQueries([
            ...
            {
                raw: {
                    //reconstruction of the team array with the lookup values (as if we used the populate).
                    $addFields: {
                        team: {
                            $map: {
                                input: "$team",
        ...
        await media.populate(results, {
            path: "equipment.equipment.mainImage",
        });
        ...
        await users.populate(results, {
            path: "meta.requestedBy",
            select: userAppModel.publicFields(),
        });
        await users.populate(results, {
            path: "meta.lastModifiedBy",
            select: userAppModel.publicFields(),
        });
